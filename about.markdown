...
지원자는 로그인 없이 공개 링크로, 담당자·면접관은 JWT(역할 3종)로 접근한다. React(Vercel) 프론트가 FastAPI(EC2 · Docker) API를 호출하고, 이력서는 presigned URL로 브라우저에서 S3에 직접 업로드된다. 단계 변경 메일은 API 가 n8n 웹훅으로 넘기고 SMTP 로 발송한다 — n8n 이 죽어도 API 가 5분마다 밀린 메일을 재발송한다(최대 3회). AWS 는 8종에서 3종으로 줄였고(ADR-0031·0036), 메일·큐·스토리지는 환경변수 스위치(MAIL_DISPATCH · QUEUE_BACKEND · S3_ENDPOINT_URL)로 공급자를 바꿀 수 있다.
